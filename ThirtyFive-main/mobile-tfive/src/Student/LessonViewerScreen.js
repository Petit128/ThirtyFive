...
export default function LessonViewerScreen({ route, navigation }) {
  ...
  const loadLesson = async () => {
    try {
      ...
      console.log('📄 Type:', response.data.lesson.file_type);
      console.log('📝 HTML présent:', !!response.data.lesson.html_content);
  ...
  // Générer le HTML complet avec CSS et JS intégrés
  const getFullHtml = () => {
    if (!lesson?.html_content && !lesson?.file_path) {
      return `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    // Si c'est un fichier HTML uploadé, on utilise son contenu
    if (lesson.file_path && !htmlContent) {
      // Le contenu est déjà chargé dans lesson.html_content
      htmlContent = lesson.html_content || '<div>Contenu interactif</div>';
    }
    
    // Construire le HTML final avec les styles et scripts
    return `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          * {
          ...
          canvas, .simulation, .interactive-element {
            display: block;
            margin: 20px auto;
            max-width: 100%;
            border-radius: 8px;
        ...
        ${lesson.css_files?.map(css => `<link rel="stylesheet" href="http://192.168.181.95:5000${css.file_path}">`).join('')}
      </head>
      <body>
        ...
        <script>
          ...
          // Émettre un événement pour indiquer que la page est chargée
        ...
        ${lesson.js_files?.map(js => `<script src="http://192.168.181.95:5000${js.file_path}"></script>`).join('')}
      </body>
      </html>
    `;
  };
  ...
  const handleMessage = (event) => {
    try {
      ...
      console.log('Message du WebView:', data);
    } catch (error) {
      console.log('Message brut:', event.nativeEvent.data);
    }
  };
      ...
      <ScrollView style={styles.content}>
        <View style={styles.infoSection}>
          <View style={styles.meta}>
            ...
            <Text style={styles.metaItem}>👁️ {lesson.views || 0} vues</Text>
            <Text style={styles.metaItem}>📥 {lesson.downloads || 0} téléchargements</Text>
          </View>
        ...
        <View style={styles.contentSection}>
          ...
          <View style={styles.webviewContainer}>
            <WebView
              ...
              onLoadEnd={() => console.log('WebView chargé')}
            />
          </View>
        </View>
